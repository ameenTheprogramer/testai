...
def click_element_by_class_name(class_name):
    try:
        ...
        element_location = pyautogui.locateCenterOnScreen('path/to/element.png', confidence=0.8)  # You need to capture the element image first
        if element_location:
            pyautogui.click(element_location.x, element_location.y)
            print(f"Successfully clicked the element with class '{class_name}'.")
        else:
            print(f"Element with class '{class_name}' not found on the screen.")
